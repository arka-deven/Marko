...
import { beehiiv, isBeehiivConfigured } from "@/lib/beehiiv/client"
...
export const onIdeaApproved = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // If an email asset was generated and Beehiiv is configured, create a draft newsletter
    if (isBeehiivConfigured() && generatedAssets.some(a => a.asset_type === "email" && a.success)) {
      await step.run("create-beehiiv-draft", async () => {
        const supabase = createAdminClient()

        // Find the email asset we just generated
        ...
        if (!emailAsset?.body) return

        try {
          const result = await beehiiv.createDraft({
            title: emailAsset.title ?? idea!.title,
            content: emailAsset.body,
          })

          // Store the Beehiiv post ID in the asset metadata for future reference
          const updatedMetadata = {
            ...(emailAsset.metadata as Record<string, unknown>),
          ...
          logger.info({ ideaId, beehiivPostId: result.data.id }, "[on-idea-approved] Beehiiv draft created")
        } catch (err) {
          logger.error({ err, ideaId }, "[on-idea-approved] Beehiiv draft creation failed — email asset still available")
          // Don't fail the whole function — the email content is still usable without Beehiiv
        }
      })
    }
