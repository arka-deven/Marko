import type { IdeaTheme, CialdiniQuote } from "@/lib/ai/cialdini-brand"
import { AWARENESS_RULES, STORYBRAND_FRAMEWORK, getQuotesForTheme, CURRENT_TRENDS } from "@/lib/ai/cialdini-brand"
...
Academic precision + practical application. No hype. No hollow promises. Specific study names, real percentages, named researchers where possible. The tone is confident and educational — never salesy. Cialdini's own writing is the model: rigorous, anecdotal, actionable.

Always respond with valid JSON only — no markdown fences, no preamble.`


// ─────────────────────────────────────────────────────────────────────────────
// TREND-JACKING — builds the prompt block that tells the AI to tie trending
// topics to Cialdini's books using real quotes as content hooks.
// Pattern: "Here's what's trending -> Here's what Cialdini says -> Subscribe"
// ─────────────────────────────────────────────────────────────────────────────

function buildTrendJackingBlock(theme: IdeaTheme, quotes: CialdiniQuote[]): string {
  const trendsList = CURRENT_TRENDS.map((t, i) => `${i + 1}. ${t}`).join("\n")
  const quotesList = quotes
    .map((q, i) => `${i + 1}. "${q.quote}"\n   — Dr. Robert Cialdini, *${q.book}* (Chapter: ${q.chapter}) [${q.principle}]`)
    .join("\n\n")

  return `## TREND-JACKING RULE (apply to at least 2 of 8 ideas)

The most viral content connects a CURRENT TREND to timeless science. At least 2 of the 8 ideas MUST be "trend-reactive" — they reference a macro trend happening right now and explain it through Cialdini's lens.

**The formula:** "Here's what's trending in business/entrepreneurship right now -> Here's what Cialdini wrote about this exact dynamic in his books -> Subscribe to learn more / Get the book."

This works because:
- Trending topics guarantee relevance and search/social discovery
- Cialdini's principles provide the "why" that no one else can articulate
- The combination positions the Cialdini Institute as the only brand that can decode trends through 35 years of persuasion research

### Current macro trends to consider (pick the 2-3 most relevant to this week's theme: **${theme}**):

${trendsList}

### Cialdini book quotes to use as content hooks this week:

These are real passages from Dr. Cialdini's books. Use them as the "bridge" between the trend and the insight. The quote IS the content hook — it proves the science predicted this trend decades ago.

${quotesList}

### How to structure a trend-reactive idea:

1. **HOOK** — Name the trend with a specific, current data point or observation (e.g., "63% of sales teams now use AI for outreach")
2. **BRIDGE** — Drop a Cialdini quote that explains WHY this trend works (or fails) from a persuasion science perspective
3. **INSIGHT** — Explain what most people get wrong about this trend because they don't understand the underlying principle
4. **CTA** — "Cialdini predicted this in [Influence/Pre-Suasion]. Here's the full framework." -> Drive to book, newsletter, or POP Workshop

### Trend-reactive idea requirements:
- The "trendReactive" field MUST be set to true for trend-jacking ideas
- The "trendTopic" field names the specific trend being referenced
- The "cialdiniBridgeQuote" field contains the exact Cialdini quote used as the bridge
- The rationale must explain HOW this week's principle (${theme}) connects the trend to Cialdini's research
- Do NOT force a trend connection — if a trend doesn't naturally connect to ${theme}, skip it and pick one that does`
}
...
// 5. Trend-Jacking — at least 2 ideas per batch tie current market trends to
//    Cialdini's books using real quotes as the content hook/bridge.
// ─────────────────────────────────────────────────────────────────────────────
...
Interpret this brief and make every idea angle, hook, and rationale directly support it. If the brief names a specific product (book, POP Workshop, CMCT, Keynote), weight ideas toward that product. If it names a specific audience segment, persona, or campaign goal, tailor the ideas to that. The brief takes priority over the default audience/stage distribution when they conflict.`
  ...
  // ── Trend-jacking: Cialdini quotes + current trends as content hooks ──
  const quotes = getQuotesForTheme(theme)
  const quotesBlock = buildTrendJackingBlock(theme, quotes)
...
[
  {
    ...
    "trendReactive": false,
    "trendTopic": null,
    "cialdiniBridgeQuote": null
  }
]

**For trend-reactive ideas (at least 2 of 8), set:**
- "trendReactive": true
- "trendTopic": "name of the macro trend being referenced"
- "cialdiniBridgeQuote": "the exact Cialdini quote used as the content bridge"
