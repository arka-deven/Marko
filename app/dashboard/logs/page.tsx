...
import { ACTOR_STYLES } from "@/lib/ui-config"
...
function ActorBadge({ actor }: { actor: "admin" | "engine" }) {
  const style = ACTOR_STYLES[actor] ?? ACTOR_STYLES.admin
  return (
    <Badge variant="secondary" className={`${style.bg} ${style.text} text-[10px] font-semibold uppercase tracking-wider border-transparent`}>
      {actor === "engine" ? "Engine" : "Admin"}
    </Badge>
  )
}
...
export default async function LogsPage() {
  ...
  return (
    <div className="space-y-6">
      {logs.length === 0 ? (
        <Card className="border-dashed bg-card/50 py-20">
